(module
  ...
  (func $sortByLen (param $arr i32) 
    ...
    (block $break_1
      (loop $loop_1
        (br_if $break_1 (i32.eqz (f64.lt (local.get $i) (f64.sub (i32.const 0) (f64.const 1)))))
        (block $cont_1
          (local.set $j (f64.const 0))
          (block $break_2
            (loop $loop_2
              (br_if $break_2 (i32.eqz (f64.lt (local.get $j) (f64.sub (f64.sub (i32.const 0) (f64.const 1)) (local.get $i)))))
              (block $cont_2
                (if (i32.load (i32.load (i32.add (i32.add (local.get $arr) (i32.const 8)) (i32.shl (i32.gt_s (;? j].length ;) (i32.const 0) (i32.add (;? arr[j ;) (i32.const 0) (i32.const 1))) (i32.const 2)))))
                  (then
  ...
  (func $sortPersonsByAge (param $arr i32) 
    ...
    (block $break_3
      (loop $loop_3
        (br_if $break_3 (i32.eqz (f64.lt (local.get $i) (f64.sub (i32.const 0) (f64.const 1)))))
        (block $cont_3
          (local.set $j (f64.const 0))
          (block $break_4
            (loop $loop_4
              (br_if $break_4 (i32.eqz (f64.lt (local.get $j) (f64.sub (f64.sub (i32.const 0) (f64.const 1)) (local.get $i)))))
              (block $cont_4
                (if (i32.gt_s (;? arr[j].age ;) (i32.const 0) (i32.add (;? arr[j ;) (i32.const 0) (;? 1].age ;) (i32.const 0)))
                  (then
                  (local.set $tmp (i32.load (i32.add (local.get $arr) (i32.shl (local.get $j) (i32.const 2)))))
                  (i32.store (i32.add (local.get $arr) (i32.shl (local.get $j) (i32.const 2))) (i32.load (i32.add (local.get $arr) (i32.shl (f64.add (local.get $j) (f64.const 1)) (i32.const 2)))))
                  (i32.store (i32.add (local.get $arr) (i32.shl (f64.add (local.get $j) (f64.const 1)) (i32.const 2))) (local.get $tmp))
                  )
                )
              )
              (local.set $j (f64.add (local.get $j) (f64.const 1)))
  ...
  (func $_start (export "_start")
    ...
    (local.set $fruits (i32.const 261))
    (call $sortByLen (local.get $fruits))
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__i32_to_str (call $strArrStr (local.get $fruits)) (i32.const 132)))
    ...
    (block $break_5
      (loop $loop_5
        (br_if $break_5 (i32.eqz (f64.lt (local.get $i) (i32.load (local.get $people)))))
        (block $cont_5
          (if (f64.gt (local.get $i) (f64.const 0))
            (then
            (local.set $pStr (i32.add (local.get $pStr) (;? " " ;) (i32.const 0)))
            )
          )
          (local.set $pStr (i32.add (local.get $pStr) (;? `{${people[i].name} ${people[i].age}}` ;) (i32.const 0)))
        )
        (local.set $i (f64.add (local.get $i) (f64.const 1)))
        (br $loop_5)
      )
