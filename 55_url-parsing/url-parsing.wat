(module
  ...
  (func $getScheme (param $u_ptr i32) (param $u_len i32) 
    ...
    (local.set $idx (;? u.indexOf(": ;) (f64.const 0))
    (return (if (result string) (f64.ge (local.get $idx) (f64.const 0)) (then (;? u.slice(0, idx) ;) (i32.const 0)) (else (;? "" ;) (i32.const 0))))
  )
  ...
  (func $getUserInfo (param $u_ptr i32) (param $u_len i32) 
    ...
    (local $hostStart f64)
    (local $__iface_tmp i32)
    (local.set $afterScheme (;? u.indexOf(": ;) (f64.const 0))
    (local.set $atIdx (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 0) (i32.const 0)))
    (local.set $hostStart (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 0) (i32.const 0)))
    (if (f64.ge (local.get $atIdx) (f64.const 0))
      (then
      (return (;? u.slice(afterScheme, atIdx) ;) (i32.const 0))
      )
    )
    (return (;? "" ;) (i32.const 0))
  )
  ...
  (func $getHost (param $u_ptr i32) (param $u_len i32) 
    ...
    (local $q f64)
    (local $__iface_tmp i32)
    (local.set $afterAt (;? u.indexOf("@") >= 0 ? u.indexOf("@") + 1 : u.indexOf(": ;) (f64.const 0))
    (local.set $end (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 0) (i32.const 0)))
    (if (f64.lt (local.get $end) (f64.const 0))
      (then
      (local.set $end (local.get $u_len))
      )
    )
    (local.set $q (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 0) (i32.const 0)))
    (if (i32.and (f64.ge (local.get $q) (f64.const 0)) (f64.lt (local.get $q) (local.get $end)))
      (then
      (local.set $end (local.get $q))
      )
    )
    (return (;? u.slice(afterAt, end) ;) (i32.const 0))
  )
  ...
  (func $getPath (param $u_ptr i32) (param $u_len i32) 
    ...
    (local $end f64)
    (local $__iface_tmp i32)
    (;; string assignment from complex expression not yet supported: host = getHost(u);)
    (local.set $hostEnd (f64.add (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (local.get $host_ptr) (local.get $host_len)) (local.get $host_len)))
    (local.set $end (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 260) (i32.const 1)))
    (if (f64.lt (local.get $end) (f64.const 0))
      (then
      (local.set $end (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 261) (i32.const 1)))
      )
    )
    (if (f64.lt (local.get $end) (f64.const 0))
      (then
    ...
    (return (;? u.slice(hostEnd, end) ;) (i32.const 0))
  )

  (func $getFragment (param $u_ptr i32) (param $u_len i32) 
    (local $idx f64)
    (local $__iface_tmp i32)
    (local.set $idx (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 261) (i32.const 1)))
    (return (if (result string) (f64.ge (local.get $idx) (f64.const 0)) (then (;? u.slice(idx + 1) ;) (i32.const 0)) (else (;? "" ;) (i32.const 0))))
  )
  ...
  (func $getRawQuery (param $u_ptr i32) (param $u_len i32) 
    ...
    (local $hIdx f64)
    (local $__iface_tmp i32)
    (local.set $qIdx (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 260) (i32.const 1)))
    (if (f64.lt (local.get $qIdx) (f64.const 0))
      (then
      (return (;? "" ;) (i32.const 0))
      )
    )
    (local.set $hIdx (call $__str_indexof (local.get $u_ptr) (local.get $u_len) (i32.const 0) (i32.const 0)))
    (return (if (result string) (f64.ge (local.get $hIdx) (f64.const 0)) (then (;? u.slice(qIdx + 1, hIdx) ;) (i32.const 0)) (else (;? u.slice(qIdx + 1) ;) (i32.const 0))))
  )
  (func $_start (export "_start")
    ...
    (local $hostParts i32)
    (local $__iface_tmp i32)
    (;; string assignment from complex expression not yet supported: rawURL = "postgres:;)
    (;; string assignment from complex expression not yet supported: scheme = getScheme(rawURL);)
    (;; string assignment from complex expression not yet supported: userInfo = getUserInfo(rawURL);)
    (;; string assignment from complex expression not yet supported: host = getHost(rawURL);)
    (;; string assignment from complex expression not yet supported: path = getPath(rawURL);)
    (;; string assignment from complex expression not yet supported: fragment = getFragment(rawURL);)
    (;; string assignment from complex expression not yet supported: rawQuery = getRawQuery(rawURL);)
    (local.set $userParts (call $__str_split (local.get $userInfo_ptr) (local.get $userInfo_len) (i32.const 262) (i32.const 1)))
    (local.set $hostParts (call $__str_split (local.get $host_ptr) (local.get $host_len) (i32.const 262) (i32.const 1)))
        ...
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (drop (call $fd_write
            ...
            (i32.const 128)))
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__i32_to_str (if (result i32) (f64.gt (i32.load (local.get $userParts)) (f64.const 1)) (then (i32.load (i32.add (i32.add (local.get $userParts) (i32.const 8)) (i32.shl (i32.const 1) (i32.const 2))))) (else (;? "" ;) (i32.const 0))) (i32.const 132)))
          (i32.store8 (i32.add (i32.const 132) (i32.load (i32.const 4))) (i32.const 10))
          (i32.store (i32.const 4) (i32.add (i32.load (i32.const 4)) (i32.const 1)))
          (drop (call $fd_write
            (i32.const 1)
        ...
        (i32.store (i32.const 0) (i32.const 132))
          ...
          (drop (call $fd_write
            ...
            (i32.const 128)))
        (i32.store (i32.const 0) (i32.const 132))
          (i32.store (i32.const 4) (call $__i32_to_str (if (result i32) (f64.gt (i32.load (local.get $hostParts)) (f64.const 1)) (then (i32.load (i32.add (i32.add (local.get $hostParts) (i32.const 8)) (i32.shl (i32.const 1) (i32.const 2))))) (else (;? "" ;) (i32.const 0))) (i32.const 132)))
          (i32.store8 (i32.add (i32.const 132) (i32.load (i32.const 4))) (i32.const 10))
          (i32.store (i32.const 4) (i32.add (i32.load (i32.const 4)) (i32.const 1)))
          (drop (call $fd_write
            (i32.const 1)
  ...
  (data (i32.const 260) "\3f")
  (data (i32.const 261) "\23")
  (data (i32.const 262) "\3a")
  (data (i32.const 263) "\6d\61\70\5b\6b\3a\5b\76\5d\5d\0a")
  (data (i32.const 274) "\76\0a")
)