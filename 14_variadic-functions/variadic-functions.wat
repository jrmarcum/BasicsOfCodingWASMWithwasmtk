(module
  ...
  (func $sum (param $nums i32) 
    ...
    (block $break_0
      (loop $loop_0
        (br_if $break_0 (i32.eqz (f64.lt (local.get $i) (i32.load (local.get $nums)))))
        (block $cont_0
          ...
          (local.set $s (i32.add (local.get $s) (;? `${nums[i]}` ;) (i32.const 0)))
        )
    ...
    (block $break_1
      (loop $loop_1
        (br_if $break_1 (i32.eqz (f64.lt (local.get $i) (i32.load (local.get $nums)))))
        (block $cont_1
          (local.set $total (f64.add (local.get $total) (f64.load (i32.add (i32.add (local.get $nums) (i32.const 8)) (i32.shl (local.get $i) (i32.const 3))))))
        )
        (local.set $i (f64.add (local.get $i) (f64.const 1)))
        (br $loop_1)
      )
  ...
  (func $_start (export "_start")
    ...
    (local.set $__rest_ptr (call $__malloc (i32.const 72)))
      ...
      (drop (call $sum (local.get $__rest_ptr)))
    (local.set $__rest_ptr (call $__malloc (i32.const 72)))
